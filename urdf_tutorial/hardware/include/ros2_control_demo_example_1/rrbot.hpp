...
namespace ros2_control_demo_example_1
{
  ...
  public:
    ...
    hardware_interface::return_type write(
        const rclcpp::Time &time, const rclcpp::Duration &period) override;
    void readFromArduino(int arduino_serial_port);
  ...
  private:
    ...
    int arduino_serial_port_;
    std::map<std::string, int> last_joint_command_to_arduino_;
    std::thread arduino_reader_thread_;
    std::atomic<bool> stop_thread_;
    std::atomic<bool> joint_2_busy_;
    std::atomic<bool> joint_5_busy_;
  };
